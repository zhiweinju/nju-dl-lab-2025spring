...
!!! example "4. 配置镜像源（可选）"
    ...
    && curl -s -L https://mirrors.tuna.tsinghua.edu.cn/nvidia-docker/gpgkey | sudo apt-key add - \
    && curl -s -L https://mirrors.tuna.tsinghua.edu.cn/nvidia-docker/$distribution/nvidia-docker.list | sudo tee /etc/apt/sources.list.d/nvidia-docker.list
...
!!! example "2. 创建 PyTorch 环境"
    ...
    conda install pytorch==1.12.1 torchvision==0.13.1 torchaudio==0.12.1 cudatoolkit=10.2 pytorch
    ~~~
...
!!! example "2. 在 VSCode 中使用（推荐）"
    ...
    2. **选择Python环境**：
       ...
       ![img](https://cdn.jsdelivr.net/gh/zhiweinju/nju-dl-lab-2025spring@main/docs/lab1/pics/2f225a81841db2a0871d40dba8853d88.png)
...
!!! example "4. 运行导入的镜像"
    ```bash
    docker run -it --name restored-pytorch-env -v ~/:/workspace my-pytorch-image:latest /bin/bash
    ```
